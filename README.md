...
Analisis data penjualan ritel Apple menggunakan SQL untuk menjawab 12 pertanyaan bisnis di tiga area: kinerja penjualan, siklus hidup produk, dan risiko garansi — dengan proses validasi kualitas data yang ketat di setiap tahap.
...
- [Skema Data (ERD)](#️-skema-data)
- [Teknik SQL yang Digunakan](#-teknik-sql-yang-digunakan)
- [Ringkasan Temuan Data Quality](#️-ringkasan-temuan-data-quality)
- [Bagian 1: Sales Performance](#-bagian-1-kinerja-keuangan--perilaku-pasar-sales-performance)
- [Bagian 2: Product Lifecycle](#-bagian-2-strategi--siklus-hidup-produk-product-lifecycle)
- [Bagian 3: Warranty/Risk Evaluation](#️-bagian-3-evaluasi-risiko--pengendalian-mutu-warrantyrisk-evaluation)
...
Setiap file `.sql` pada folder `queries/` memuat beberapa query yang dipisahkan komentar section (misal `-- Analysis 1a`). File `01-05` mencakup tahap setup, import, dan pembersihan data sebelum masuk ke tahap analisis (`06-08`).
...
```mermaid
erDiagram
    STORES ||--o{ SALES : "menjual di"
    PRODUCTS ||--o{ SALES : "terjual sebagai"
    CATEGORY ||--o{ PRODUCTS : "mengelompokkan"
    SALES ||--o{ WARRANTY : "memicu klaim"
...
| Kategori | Teknik |
|---|---|
| **Window Functions** | `LAG()` (analisis YoY, tren toko), `RANK()` (ranking bulan/produk/toko), agregasi window (`SUM() OVER`) |
| **CTE (Common Table Expressions)** | Multi-level CTE untuk memecah logika kompleks (contoh: 4-5 CTE berlapis pada analisis kanibalisasi produk) |
| **Join** | `INNER JOIN`, `LEFT JOIN`, self-join (deteksi & penggabungan toko duplikat) |
| **Aggregate Functions** | `SUM`, `AVG`, `COUNT`, `PERCENTILE_CONT` (median) |
| **Conditional Logic** | `CASE WHEN` untuk kategorisasi (cohort produk, klasifikasi premium) |
| **Date Arithmetic** | Perhitungan selisih tanggal, konversi ke bulan absolut untuk perbandingan time-series |
| **Data Validation** | Query diagnostik untuk deteksi anomali (proporsi data tidak valid, duplikasi entri) sebelum analisis utama dijalankan |
